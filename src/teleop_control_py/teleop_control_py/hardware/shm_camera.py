...
def build_camera_spec(camera_type: str, serial_number: str = "") -> CameraShmSpec:
    normalized_type = normalize_camera_type(camera_type)
    normalized_serial = str(serial_number).strip()
    identity = f"{normalized_type}:{normalized_serial or 'default'}"
...
class ShmCameraClient(BaseCamera):
    ...
    def __init__(
        ...
        *,
        ...
        self.spec = build_camera_spec(camera_type, serial_number)
        self._logger = logger
